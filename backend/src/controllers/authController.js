...
exports.registerUser = async (req, res) => {
  try {
    ...
    const { name, email, password, phone, role, address, siret } = req.body;
    const requestedRole = role ? role.toLowerCase() : UserRoles.CLIENT;

    if (requestedRole === UserRoles.EMPLOYEE) {
      return res.status(400).json({ message: 'Les employés doivent être créés par une entreprise.' });
    }
    ...
    const user = new User({
      ...
      vehiclePlate: requestedRole === UserRoles.CLIENT ? req.body.vehiclePlate : undefined,
      vehicleSerialNumber: requestedRole === UserRoles.CLIENT ? req.body.vehicleSerialNumber : undefined,
      vehicleType: requestedRole === UserRoles.CLIENT ? req.body.vehicleType : undefined
    });
